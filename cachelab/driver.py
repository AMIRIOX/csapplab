...
def main():
    ...
    stdout_data = stdout_data.decode('utf-8')  # 将 bytes 转为 str
    stdout_data = re.split('\n', stdout_data)
    ...
    stdout_data = stdout_data.decode('utf-8')  # 转换为 str
    result32 = re.findall(r'(\d+)', stdout_data)
    ...
    trans32_score = computeMissScore(
        miss32, 300, 600, maxscore['trans32']) * int(result32[0])
    trans64_score = computeMissScore(
        miss64, 1300, 2000, maxscore['trans64']) * int(result64[0])
    trans61_score = computeMissScore(
        miss61, 2000, 3000, maxscore['trans61']) * int(result61[0])
    csim_cscore = list(map(int, csim_cscore))  # 转换为 list
    total_score = csim_cscore[0] + trans32_score + \
        trans64_score + trans61_score
