...
/* Token types */
typedef enum { TOK_IDENT, TOK_NUM, TOK_REG, TOK_INSTR, TOK_PUNCT, TOK_ERR } token_t;
            ...
            for (i = 0; tcount && i < bcount; i++) {
                if (block_factor) {
                    fprintf(out, "    bank%d[%d] = 8\'h%.2x;\n", (pos + i) % block_factor,
                            (pos + i) / block_factor, code[i] & 0xFF);
                } else {
...
unsigned long long atollh(const char *p) { return strtoull(p, (char **)NULL, 16); }
